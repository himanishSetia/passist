...
const listenPort = process.env.port || process.env.PORT || process.env.Port || 3978;
...
var rClients = require('restify-clients');
...
bot.dialog('SubmitTicket', [
    ...
    (session, result, next) => {
        if (result.response) {
            ...
            const client = rClients.createJsonClient({ url: ticketSubmissionUrl });
...
bot.dialog('ExploreKnowledgeBase', [
    (session, args) => {
        var category = builder.EntityRecognizer.findEntity(args.intent.entities, 'Category');
        console.log("args.intent.entitiesargs.intent.entitiesargs.intent.entities ",args.intent.entities)
        console.log("_+_++_+_+_+_+_+_+_+_+_+_+ ",category)
        if (!category) {
